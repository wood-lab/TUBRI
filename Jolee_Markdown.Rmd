...
# check that code runs with new dataset
...
# scatter plot
# code works with fake data, check with new full.data.stages dataset
ggplot(full.data.stages %>%
         filter(psite_taxon != "NEM"),  # it should be NEM right?
       aes(x = psite_count,
           y = fcf,
           colour = life_stage,
           shape = psite_taxon)) +
  ...
  #scale_color_manual(values = c("Adult" = "blue", "Juvenile" = "red"))
  theme_classic()


# boxplot - wrong and probably won't use (colour code is incorrect)
ggplot(full.data.revised %>%
         filter(psite_taxon != "CEST"),
       aes(x = life_stage,
           y = fcf,
           colour = psite_taxon,
           scale_colour_gradient(psite_count, low = "blue", high = "red", name = "Parasite Count"))) + # fix to add in psite_count
  geom_boxplot() +
  xlab("Life Stage") +
  ylab("FCF") +
...
```{r model for q3}
#<<<<<<< HEAD
summary(q3m <- lm(fcf ~ life_stage * psite_taxon + psite_count, data = full.data.revised)) # code no longer works b/c "revised"
plot(q3m)

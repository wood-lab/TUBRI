...
### Plot with FCF, life stage, taxon (T & C), and parasite count to answer question 3)
```{r, plots for Question 3}
# FCF ~ life_stage * taxon + psite_count (A: T vs C, L: T vs C)

# Jolee's plots - messing around

# scatter plot with all four factors
ggplot(full.data.revised %>%
         filter(psite_taxon != "CEST"), # or is it NEM that we're taking out?
       aes(x = psite_count,
           y = fcf,
           colour = life_stage,
           shape = psite_taxon)) +
...
# maybe boxplot without adding parasite count - not correct code
ggplot(full.data.revised %>%
         filter(psite_taxon != "CEST"),
       aes(x = life_stage,
           y = fcf,
           colour = psite_taxon,
           scale_colour_gradient(psite_count, low = "blue", high = "red", name = "Parasite Count"))) + # fix add psite_count
  geom_boxplot() +
  xlab("Life Stage") +
  ylab("FCF") +

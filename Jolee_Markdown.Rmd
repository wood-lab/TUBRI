...
### _Hybognathus nuchalis_ - add columns to identify species as an adult or larval stage and new column for total parasite count
```{r h. nuch a/l stage}
Hnuc.life.stage <- H.nuchalis.data %>%
  mutate(larval = rowSums(select(., 
                      "nem_daft",
                      "trem_cm",
                      "trem_meta_het",
...
Hnuc.life.stage <- Hnuc.life.stage %>%
  mutate(body.index.nuc = ((Hnuc.life.stage$weight_mg/((Hnuc.life.stage$standard_length_mm)^3)))*(100)) %>%
  janitor::clean_names() %>%
  filter(sex != "MA" & sex != "")
...
```{r}
na_ip_pv_hn_combined <- bind_rows(Nath.life.stage, Pvig.life.stage, Ipun.life.stage, Hnuc.life.stage, .id = "dataset")
na_ip_pv_hn_pp <- preProcess(na_ip_pv_hn_combined, method = c("center", "scale"))
na_ip_pv_hn_scaled <- predict(na_ip_pv_hn_pp, na_ip_pv_hn_combined)
notath_scaled <- na_ip_pv_hn_scaled %>% filter(dataset == "1") %>%
  mutate(body.index.notath = ((Nath.life.stage$weight_mg/((Nath.life.stage$standard_length_mm)^3)))*(100)) %>%
  janitor::clean_names()
pimvig_scaled <- na_ip_pv_hn_scaled %>% filter(dataset == "2") %>%
  mutate(body.index.pimvig = ((Pvig.life.stage$weight_mg/((Pvig.life.stage$standard_length_mm)^3)))*(100)) %>%
  janitor::clean_names()
ictpun_scaled <- na_ip_pv_hn_scaled %>% filter(dataset == "3") %>%
  mutate(body.index.ictpun = ((Ipun.life.stage$weight_mg/((Ipun.life.stage$standard_length_mm)^3)))*(100)) %>%
  janitor::clean_names()
hybnuc_scaled <- na_ip_pv_hn_scaled %>% filter(dataset == "4") %>%
  mutate(body.index.hybnuc = ((Hnuc.life.stage$weight_mg/((Hnuc.life.stage$standard_length_mm)^3)))*(100)) %>%
  janitor::clean_names()
```
...
HN.psc.fcf <- ggplot(Hnuc.life.stage, 
...
HN.bc.a <- ggplot(Hnuc.life.stage, 
       aes(x = adult, y = body_index_nuc, color = sex)) + xlab("Adult")  + geom_point() +
  ylab("Fulton's Condition Factor")  + theme_bw() + stat_smooth(method=lm)

HN.bc.l <- ggplot(Hnuc.life.stage,

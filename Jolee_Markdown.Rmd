...
```{r CW datasets, eval=FALSE, echo=FALSE}
...
body.index.vig <- ((P.vigilax.data$weight_mg/((P.vigilax.data$standard_length_mm)^3)))*(100) # still need to edit this
body.index.ath <- ((Nath.life.stage$weight_mg/((Nath.life.stage$standard_length_mm)^3)))*(100)
body.index.pun <- ((I.punctatus.data$weight_mg/((I.punctatus.data$standard_length_mm)^3)))*(100) # still need to edit this
```
...
NA.bc.fcf <- ggplot(Nath.life.stage, 
       aes(x = BinaryCode, y = body.index.ath)) + xlab("Life Stage")  + geom_boxplot() +
...
summary(FCF_psite_ath_lm <- lm(body.index.ath ~ psite_count, 
                      data = N.atherinoides.data, family = 'nbinom2'))
plot(FCF_psite_ath)
# Question 1 (ANOVA)
summary(FCF_psite_ath_ANOVA <- aov(body.index.ath ~ factor(psite_count), data = N.atherinoides.data))

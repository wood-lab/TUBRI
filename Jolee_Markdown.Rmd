...
H.nuchalis.data <- read_csv("data/processed/Hybognathus_nuchalis_processed_human_readable.csv") %>%
  janitor::clean_names()
P.vigil.data <- read_csv("data/processed/Percina_vigil_processed_human_readable.csv") %>% 
  janitor::clean_names()
```

#### Mutate character variable to numeric for Hnuc and Pvigil
...
PV.psc.fcf <- ggplot(Pvig.life.stage.total, 
       aes(x = psite_count, y = body_index_vig, colour = sex)) + geom_point() + xlab("Parasite Count")  + 
  ylab("FCF")  + theme_bw() + stat_smooth(method=lm)
...
$$
K_i\sim\ life\ stage\ * taxon\ + parasite\ count
$$

#### Run linear model to test above questions
```{r model}
## we can also run these using the total dataset and then make fish species another random effect
# Question 1 (lm)
...
```{r adult vs larval splits, echo=FALSE}
full.data.stages <- full.data %>%
  mutate(life_stage = ifelse(psite_spp_x %in% c("META.HS",
                      ...
                      "TREM.META.UNK"), # H. nuchalis ends
                            "Larval", "Adult"),
    psite_taxon = ifelse(sub("\\..*", "", psite_spp_x) == "META", "TREM", sub("\\..*", "", psite_spp_x))
  ) %>%
  janitor::clean_names()
```
...
```{r only a/l both in host, echo=FALSE}
...
```{r FCF into new dataframe, echo=FALSE}
...
full.data.revised$psite_taxon <- as.factor(full.data.revised$psite_taxon)
full.data.revised$life_stage <- as.factor(full.data.revised$life_stage)


summary(q3m <- lm(fcf ~ life_stage*psite_taxon-1, data = full.data.revised))
plot(q3m)
```

#### Remove outliers from the final model and rerun
```{r}
# Remove rows with missing values in the relevant columns
full.data.revised_cleaned <- full.data.revised %>%
  filter(!is.na(fcf) & !is.na(life_stage) & !is.na(psite_taxon) & !is.na(psite_count))

# Fit the model
q3m <- lm(fcf ~ life_stage * psite_taxon + psite_count, data = full.data.revised_cleaned)

# Augment the cleaned data with model diagnostics
full.data.revised_augmented <- augment(q3m, data = full.data.revised_cleaned)

# Visualize standardized residuals
ggplot(full.data.revised_augmented, aes(x = .fitted, y = .std.resid)) +
  geom_point() +
  geom_hline(yintercept = c(-3, 3), color = "red", linetype = "dashed") +
  theme_minimal() +
  ggtitle("Standardized Residuals")

# Filter out outliers with standardized residuals > |3|
full.data.revised_no_outliers <- full.data.revised_augmented %>%
  filter(abs(.std.resid) <= 3)

# Re-run the model without outliers
summary(clean_model <- lm(fcf ~ life_stage * psite_taxon + psite_count, data = full.data.revised_no_outliers))

# Visualize the clean data
ggplot(full.data.revised_no_outliers, aes(x = .fitted, y = fcf)) +
  geom_point()

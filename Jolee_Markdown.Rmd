...
full.data.stages <- full.data.stages %>%
  mutate(FCF = ((((full.data.stages$weight_mg)/1000)/((full.data.stages$standard_length_mm)^3)))*(100)) %>% 
...
### change data to include the total adults and larvae we found
```{r}
# Question 2 (lm) 
summary(q2m <- lmer(fcf ~ relevel(life_stage, ref = "uninfected") + psite_count + (1|fish_sp_x), data = full.data.stages))
...
# run the model
summary(q3m <- lmer(fcf ~ life_stage + psite_count + (1|fish_sp_x)+ (1|catalog_number), data = full.data.stages))

# check colinearity using vif function --> GVIF^(1/(2*Df)) less than 10 means model is ok concerning colinearity (under 5 is acceptable generally)
vif(q3m)

# check how the model fits
s3=simulateResiduals(fittedModel=q3m,n=250)
s3$scaledResiduals
plot(s3)
...
mydf <- ggpredict(q3m, c("life_stage"), jitter=TRUE) 

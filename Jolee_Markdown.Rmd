...
```{r q1m}
# run models 
summary(fcf_pimvig_lm <- lm(body_index_pimvig ~ psite_count + ci, data = P.vigilax.data.fcf)) ### @ a = 0.05
summary(fcf_ictpun_lm <- lm(body_index_ictpun ~ psite_count + ci, data = I.punctatus.data.fcf))
summary(fcf_notath_lm <- lm(body_index_notath ~ psite_count + ci, data = N.atherinoides.data.fcf))
...
# sig plot from above for pimvig
...
ggplot(combined_data_long, aes(x = psite_count, y = body_index_value, color = ci)) + 
  geom_point() + 
  xlab("Parasite Count") + 
  ylab("Body Index") + 
  theme_bw() + 
...
## still contains percina -- need to remove percina
...
mydf <- ggpredict(q2m, c("ci", "life_stage"))
...
plot(mydf, rawdata = TRUE, alpha = 0.3, dot.alpha = 0.6,colors=c("#969696","#ce1256","blue")) + jitter("predicted", factor = 0.001, amount = NULL) +

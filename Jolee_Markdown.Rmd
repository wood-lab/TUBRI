...
### Plot body indexes 
```{r plot FCF, eval = FALSE, echo=FALSE}
plot(body.index.vig)
...
IP.psc.fcf <- ggplot(I.punctatus.data, 
       aes(x = body.index.pun, y = psite_count, fill = ci)) + geom_boxplot() + xlab("FCF")  + 
  ylab("psite count")  + theme_bw()

NA.psc.fcf <- ggplot(N.atherinoides.data, 
       aes(x = body.index.ath, y = psite_count, fill = ci)) + geom_boxplot() + xlab("FCF")  + 
  ylab("psite count")  + theme_bw()

PV.psc.fcf <- ggplot(P.vigilax.data, 
       aes(x = body.index.vig, y = psite_count, fill = ci)) + geom_boxplot() + xlab("FCF")  + 
  ylab("psite count")  + theme_bw()
...
ggplot(Nath.life.stage, # plot doesn't show the two different plots, just the same one twice
...
```{r model}
# Question 1 (lm)
summary(FCF_psite_ath_lm <- lm(body.index.ath.stage ~ psite_count, 

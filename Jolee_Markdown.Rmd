...
unique_fish_fcf <- full.data.stages %>%
  select(individual_fish_id, fish_sp_x, standard_length_mm, weight_g, fcf) %>%
  distinct()
...
combined_data <- bind_rows(
  ...
  I.punctatus.data.fcf %>% mutate(species = "I.punctatus"),
  N.atherinoides.data.fcf %>% mutate(species = "N.atherinoides"),
  H.nuchalis.data.fcf %>% mutate(species = "H.nuchalis")
)
```
...
```{r q1m}
# run models 
summary(fcf_pimvig_lm <- lm(body_index_pimvig ~ psite_count, data = P.vigilax.data.fcf))
summary(fcf_ictpun_lm <- lm(body_index_ictpun ~ psite_count, data = I.punctatus.data.fcf))
summary(fcf_notath_lm <- lm(body_index_notath ~ psite_count, data = N.atherinoides.data.fcf))
summary(fcf_hybnuc_lm <- lm(body_index_hybnuc ~ psite_count, data = H.nuchalis.data.fcf))
```
...
combined_data_long <- combined_data %>%
  gather(key = "body_index_type", value = "body_index_value", body_index_ictpun, body_index_notath, body_index_pimvig, body_index_hybnuc)

ggplot(combined_data_long, aes(x = psite_count, y = body_index_value, color = species)) + 
  geom_point() + 
  xlab("Parasite Count") + 
  ylab("Body Index") + 
  theme_bw() + 
...
# run the model
summary(q2m <- lmer(fcf ~ life_stage + psite_count + (1|fish_sp_x)+ (1|catalog_number), data = full.data.stages))
...
mydf <- ggpredict(q2m, c("life_stage"), jitter=TRUE) 
...
plot(mydf, rawdata = TRUE, alpha = 0.3, dot.alpha = 0.6,colors=c("#969696","#ce1256")) +
  labs(x = 'Life stage', y = 'K',title=NULL)+
  apatheme 

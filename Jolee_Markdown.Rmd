...
```{r q1m}
# run models 
summary(fcf_pimvig_lm <- lm(body_index_pimvig ~ psite_count + ci, data = P.vigilax.data.fcf)) ### @ a = 0.1
...
ggplot(combined_data_long, aes(x = psite_count, y = body_index_value, color = ci)) + 
  ...
  facet_wrap(~species) +
  stat_poly_eq(aes(label = paste("p-value: ", format(..p.value.., digits = 3), sep = "")), 
               formula = y ~ x, parse = TRUE, label.x = "right", vjust = 0.5)

...
ggplot(full.data.stages,
  ...
  geom_point() +
  xlab("Parasite Count") +
  ylab(expression(paste("FCF (", mg/mm^3, ")"))) +
  theme_classic()
...
# Jolee: plot - include sex? if so, need to match f and F and F, m and M, and take out whatever the blank one is (red)
ggplot(full.data.stages %>%
         filter(sex != "MA") %>%
         filter(sex != "NA") %>%
         filter(sex != "na") %>%
...
# scatterplot w/ both adult and larval (hard to see adult)
ggplot(full.data.stages %>%
         filter(psite_taxon != "nem") %>%
         filter(psite_taxon != "uninfected") %>%
         filter(life_stage != "uninfected"),
       aes(x = psite_count,
           y = fcf,
           colour = life_stage,
           shape = psite_taxon)) +
  geom_point(size = 3) +
  labs(x = "Parasite Count",
       y = expression(paste("FCF (", mg/mm^3, ")")),
       colour = "Life Stages",
       shape = "Parasite Taxon") +
    scale_shape_manual(values = c(16, 17)) +
  theme_classic()


# scatterplot just adult - colour is not constant (change to make trem the same colour in both plots)
Q3.adult <- ggplot(full.data.stages %>%
        filter(psite_taxon != "nem") %>%
         filter(psite_taxon != "uninfected") %>%
         filter(life_stage != "larval") %>%
         filter(life_stage != "uninfected"),
       aes(x = psite_count,
           y = fcf,
           colour = psite_taxon,
  ...
  labs(x = "Parasite Count",
       y = expression(paste("FCF (", mg/mm^3, ")")),
       colour = "Parasite Taxon",
       shape = "Life Stage") +
    scale_shape_manual(values = c(16)) +
  theme_classic()

# scatterplot just larval - colour is not constant (change to make trem the same colour in both plots)
Q3.larval <- ggplot(full.data.stages %>%
         filter(psite_taxon != "nem") %>%
         filter(psite_taxon != "uninfected") %>%
...
ggarrange(Q3.adult, Q3.larval, common.legend = TRUE, legend = "right", labels = c("A", "B")) 
# missing larval legend



# boxplot - not using, missing parasite count (that part of code is wrong)
ggplot(full.data.stages %>%
         filter(psite_taxon != "uninfected") %>%
         filter(psite_taxon != "nem") %>%
         filter(life_stage != "uninfected"),
       aes(x = life_stage,
           y = fcf,
           colour = psite_taxon,
           scale_colour_gradient(psite_count, low = "blue", high = "red", name = "Parasite Count"))) + # add psite_count 
  geom_boxplot() +
  xlab("Life Stage") +
  ylab("FCF") +
